...
为长时间工作的人，打开一扇安静的窗。项目当前处于第一阶段开发与自测，尚未发布正式版本。

## 当前原型

**2026-09-17：0.1.3 收尾修正版。** 在 0.1.2 横向远眺与程序自然景物基础上，统一了景深、视差与遮挡顺序，增强远近雾感，加入独立天空风速。Windows 短回归已通过；提供可在 Mac 运行的网页包与本机编译脚本，见 [Mac 测试说明](docs/MAC_TESTING.md)。色彩体系、山型丰富度和最终审美仍待下一轮完善，2019 Intel Mac 网页版基础体验已获用户确认，原生桌面版及低配功耗仍待实测。见[视觉规范](docs/VISUAL_DIRECTION.md)与[测试移交](docs/validation/PHASE_1_HANDOFF.md)。

- 自然景物不再使用固定云山轮廓图集；运行中计算新的云团轮廓、色面、山脊和开阔区。
- 精细几何绘制的蒸汽机车、煤水车、十节细长客车；水平拱桥、栏杆、拱券与柔和蒸汽。
...
- Windows 0.1.3 测试包；2019 Intel Mac 网页基础体验正常，Mac 桌面版、低配功耗与最终审美认可仍待验证。当前以横屏为主。
...
Windows 探针还会最小化／恢复自己的测试窗口，检查原生事件是否暂停与恢复引擎。锁屏／睡眠桥接已接入 Windows；macOS 原生生命周期桥接仍待实现与实测。
...
开发服务器的 `/review.html` 默认提供 **1 分钟昼夜快速验收**，也可选择正式30分钟周期，录制时长可选1/5/30分钟。它按有效播放时间计时，使用相同场景代码；此页面不是生产构建入口，不改变正式程序的30分钟默认值，不随安装包提供。长时间功耗/稳定性测试独立安排，不阻塞每次开发交付。
